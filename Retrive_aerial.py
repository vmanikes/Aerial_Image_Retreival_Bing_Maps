...
while initialZoomLevel > 0:
    start_PixelX, start_PixelY = latLong2pixelXY(float(args[1]), float(args[2]), initialZoomLevel)
    end_PixelX, end_PixelY = latLong2pixelXY(float(args[3]), float(args[4]), initialZoomLevel)
...
for i in tile_2d:
    temp_img_matrix = []
    for j in i:
        temp_quad = tileXY2QuadKey(j[0], j[1], tile_dict[j])
        temp_img = get_image_from_quadkey(temp_quad)
        temp_img_matrix.append(temp_img)
    image_2d.append(temp_img_matrix)

img_matrix = []
for i in image_2d:
    img_matrix.append(np.concatenate(i, axis=0))

temp_mat = []
temp_mat.append(np.concatenate(img_matrix, axis=1))

cv2.imwrite("final.jpeg", temp_mat[0])
img_Final = cv2.imread('final.jpeg')
